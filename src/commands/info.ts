...
const TEMPLATE_INFO: Record<string, TemplateInfo> = {
  'nextjs-fullstack': {
    ...
    description: 'Complete full-stack application with Next.js 15 App Router, Express backend, and Prisma ORM',
    techStack: ['Next.js 15', 'React', 'Express', 'Prisma', 'PostgreSQL', 'TailwindCSS', 'TypeScript'],
    features: ['App Router', 'API Routes', 'Server Actions', 'Auth Ready', 'Docker Support', 'GitHub Actions CI'],
    files: ['package.json', 'tsconfig.json', 'next.config.ts', 'tailwind.config.ts', 'prisma/schema.prisma', 'docker-compose.yml'],
  },
  'express-api': {
    name: 'Express REST API',
    description: 'Production-ready REST API with Express, Prisma ORM, and complete authentication system',
    techStack: ['Express', 'Prisma', 'PostgreSQL', 'JWT', 'bcrypt', 'TypeScript'],
    features: ['CRUD Operations', 'JWT Authentication', 'Role-based Access', 'Input Validation', 'Error Handling', 'Docker Support'],
    files: ['package.json', 'tsconfig.json', 'prisma/schema.prisma', 'docker-compose.yml', 'src/server.ts', 'src/routes/*.ts'],
  },
  'python-fastapi': {
    name: 'Python FastAPI',
    ...
    features: ['Auto-generated Docs', 'Async Support', 'Type Validation', 'JWT Auth', 'Database Migrations', 'Docker Support'],
    files: ['pyproject.toml', 'alembic.ini', 'app/main.py', 'app/models.py', 'app/schemas.py', 'docker-compose.yml'],
  },
  'go-microservice': {
    ...
    features: ['Clean Architecture', 'Middleware System', 'Context Propagation', 'Graceful Shutdown', 'Health Checks', 'Docker Multi-stage'],
    files: ['go.mod', 'cmd/server/main.go', 'internal/handlers/*.go', 'pkg/db/*.go', 'docker-compose.yml', 'Dockerfile'],
  },
};
...
export async function infoCommand(templateName?: string) {
  ...
  console.log(chalk.bold('\nUsage:'));
  console.log(chalk.gray(`  codescaffold create my-${templateName.split('-')[0]}-project --template ${templateName}`));
